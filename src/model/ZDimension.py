...
class ZDimension(PlanComponent):
    ...
    def get_svg_style_string(self):
        style_string_list = []
        style_string_list.append(SVGHelper.gen_style_string(f'{type(self).__name__}-Text', 'fill: #00a1a1', 'font-size: 7pt', 'font-family: monospace', 'text-anchor: left'))
    ...
    def get_svg_string(self, scale_divisor):
        ...
        text_position_offset = np.array([5, 0])
        text_position = np.add(transformed_position, text_position_offset)
        
        svg_string_list = []
        svg_string_list.append(f'<text class="{type(self).__name__}-Text"  x="{text_position[0]}" y="{text_position[1]}">')
        ...
        svg_string_list.append(SVGHelper.gen_circle_string(transformed_position, 3, f'{type(self).__name__}-Dot'))
